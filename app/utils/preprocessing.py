...
def process_Htask(df): 
    ...
    missy = ['res_#', 'booking_date', 'arrival', 'dept', 'cancellation_date', 'last_modified_date']
    for column in missy:
        df[column] = df[column].apply(clean_data)

    # 0 numbers
    df['mobile_no'] = df['mobile_no'].apply(lambda x: pd.NA if pd.notna(x) and x == '0' else x)
    
    # convert to date
    date_columns = ['booking_date','arrival', 'dept', 'cancellation_date', 'last_modified_date']
    for column in date_columns:
        df[column] = pd.to_datetime(df[column], format='%d/%m/%Y')
    
    # convert to time
    df['booking_time'] = pd.to_datetime(df['booking_time'], format='%I:%M:%S %p').dt.time
    
    # make time and date in same column
    df['booking_datetime'] = df.apply(
        lambda row: datetime.datetime.combine(row['booking_date'], row['booking_time']), axis=1
    )
    ...
    df.drop(
        ['hotel_name', 'city', 'zip_code', 'state', 'preference', 'travelagent',
        'salesperson', 'market_code_', 'unnamed:_40', 'booking_date', 'reservationn_type',
        'email', 'pax', 'rate_type', 'due_amt.', 'booking_time', 'deposit', 'new_source',
        'last_modified_date', 'last_modified_by', 'number_of_rooms_booked',
        ],
        axis=1,
        inplace=True
    )
